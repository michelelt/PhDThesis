...
def plot_data(bins,bookings_enjoy,bookings_car2go,rentals_enjoy,rentals_car2go):
    ...
    plt.subplots_adjust(left=0.14, bottom=0.36, right=0.94, top=0.9,
                wspace=None, hspace=None)


    #print(bookings)
    ax.plot(bins, bookings_enjoy, color= 'r', label="Bookings Enjoy",linewidth=1.5)
    #ax.plot(bins, rentals_enjoy, color= 'r', linestyle="--", label="Rentals Enjoy")
    ax.plot(bins, bookings_car2go, color= 'b',  label="Bookings CarGo",linewidth=1.5)
    ...
    legnew = ax.legend(bbox_to_anchor=(-0.03, 1.22), loc=2, numpoints=1, frameon=None, ncol=2, prop={'size':28})
...
def main():
    ...
    bins,bookings_enjoy,rentals_enjoy = read_file("../data/enjoy_1.csv")
    bins,bookings_car2go,rentals_car2go = read_file("../data/car2go_1.csv")
